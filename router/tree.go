...
// TODO: Okay so comparing byte slices and strings are about the same in golang;
// but checking the length of the string before the string itself is ALWAYS
// faster!!
...
func RegisterMethod(method string) {
	...
	if n > strconv.IntSize {
		panic(fmt.Sprintf("[fatal error] max number of methods reached (%d)", strconv.IntSize))
	}
...
func (n *node) InsertRoute(method methodType, pattern string, handler http.Handler) *node {
	...
	for {
		...
		var segTail byte
		var segEndIdx int
		var segmentType nodeType
		var segRexpat string
		if label == '{' || label == '*' {
			segmentType, _, segRexpat, segTail, _, segEndIdx = patNextSegment(search)
		}

		var prefix string
		if segmentType == ntRegexp {
			prefix = segRexpat
		}

		// Look for the edge to attach to
		parent = n
		n = n.getEdge(segmentType, label, segTail, prefix)
...
// For a URL router like the routers's, we split the static, param, regexp and wildcard segments
...
func (n *node) addChild(child *node, prefix string) *node {
	...
	// Parse next segment
	segmentType, _, segRexpat, segTail, segStartIdx, segEndIdx := patNextSegment(search)
	...
	default:
		...
		if segStartIdx == 0 {
			...
			child.nodeType = segmentType

			if segmentType == ntCatchAll {
				segStartIdx = -1
			} else {
...
func (n *node) findPattern(pattern string) bool {
	...
	for _, nds := range nn.children {
		...
		default:
			panic("[fatal error] unknown node type")
		}
	...
	for {
		...
		for i := 0; i < len(paramKeys); i++ {
			if paramKeys[i] == paramKey {
				panic(fmt.Sprintf("[fatal error] routing pattern '%s' contains duplicate param key, '%s'", pattern, paramKey))
			}
		}
