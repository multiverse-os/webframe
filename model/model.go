...
func (self Collection) UseDB(db *database.Database) Collection {
	self.Database = db
	return self
}

func (self Collection) Id() []byte {
	total := len(self.Name)
	for index, nameRune := range self.Name {
...
func (self Collection) GenerateId() muid.Id {
	return GenerateId().Prefix(self.Name + ".")
}

func (self Collection) Keys() {
	for key := range Database.Store.Keys() {
		if len(self.Name) < len(key) && bytes.Compare(key[:len(self.Name)], []byte(self.Name)) == 0 {
...
func (self *Record) GenerateId() muid.Id {
	if self.Id.IsNil() {
		self.Id = self.Collection.GenerateId()
	}
	return self.Id
}

func (self *Record) Save() bool {
	fmt.Printf("*Instance(self) %v\n", self)
	fmt.Printf("*Instance(self.Id) %v\n", self.Id)
	fmt.Printf("*Instance(self.Instance) %v\n", self.Instance)
...
func (self *Record) New() bool {
	...
	recordBytes, _ := Encode(self)

	fmt.Printf("recordBytes(%v)", recordBytes)

	result := self.Collection.Database.Store.Put(self.Id.Bytes(), recordBytes)
